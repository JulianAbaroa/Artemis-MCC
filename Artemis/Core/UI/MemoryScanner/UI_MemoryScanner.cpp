...
void UI_MemoryScanner::Draw()
{
    ...
    this->FirstDraw();
    this->ApplyResetIfRequested("Memory Scanner");

    ImGuiWindowFlags flags = ImGuiWindowFlags_None;
    if (UI_Tab::s_Locked) flags |= ImGuiWindowFlags_NoMove;

    ImGui::Begin("Memory Scanner", &m_Visible, flags);
...
void UI_MemoryScanner::DrawKnownSizesCombo() const
{
    ...
    for (int i = 0; 
        i < (int)(sizeof(s_KnownSizes) / sizeof(s_KnownSizes[0])); 
        ++i)
    {
        ImGui::PushID(i);
        ...
        {
            snprintf(m_SizeBuf, sizeof(m_SizeBuf), 
                "0x%zX", s_KnownSizes[i].Size);
        }
...
void UI_MemoryScanner::DispatchScan()
{
    ...
    ScanDataType dt = this->CurrentDataType();
    ...
    case ScanMode::InRange:
    {
        ...
        else
        {
            m_Deps.System_MemoryScanner.TriggerInRangeScan(
                ...
                this->ParseValueBuf(m_ValueBBuf, dt), m_DelayMs);
        }
...
void UI_MemoryScanner::DrawScanStatus()
{
    ...
    {
        this->DrawRoundHistoryTooltip(session, isTyped);
    }
}

void UI_MemoryScanner::DrawRoundHistoryTooltip(
    const ScanSession& session, bool isTyped)
{
    ImGui::BeginTooltip();
    ...
    for (int r = 0; r < (int)session.Rounds.size(); ++r)
    {
        ...
        snapshotLabel(round.Before.Data.empty(), 
            "Before...", "Before OK");

        ImGui::SameLine();

        snapshotLabel(round.After.Data.empty(), 
            "After...", "After OK");
        ...
        if (round.IsComplete)
        {
            ...
            const size_t count = isTyped ? 
                round.TypedDiffs.size() : round.Diffs.size();
...
void UI_MemoryScanner::DrawModeCombo()
{
    ...
    for (int i = 0; 
        i < (int)(sizeof(s_Modes) / sizeof(s_Modes[0])); 
        ++i)
    {
        ImGui::PushID(i);
        ...
        if (ImGui::Selectable(s_Modes[i].Label, selected))
        {
            ...
            bool needsType = 
                s_Modes[i].Mode != ScanMode::Changed &&
                s_Modes[i].Mode != ScanMode::Unchanged;

            if (needsType &&
                this->CurrentDataType() == ScanDataType::Bytes)
            {
                m_DataTypeIndex = 5;
            }
        }
...
{
    const bool bytesAllowed = 
        s_Modes[m_ModeIndex].Mode == ScanMode::Changed ||
    ...
    if (!ImGui::BeginCombo("##type",
        s_DataTypes[m_DataTypeIndex].Label))
    {
        return;
    }

    for (int i = typeStart; 
        i < (int)(sizeof(s_DataTypes) / sizeof(s_DataTypes[0])); 
        ++i)
    {
        ImGui::PushID(i);
...
void UI_MemoryScanner::DrawFilters()
{
    ...
    ImGui::InputText("##from", m_FilterFromBuf, 
        sizeof(m_FilterFromBuf));
...
uintptr_t UI_MemoryScanner::ResolveAddress(
    const ScanSession& session, size_t offset)
{
    return session.Rounds.empty() ? 
        0 : session.Rounds[0].Before.BaseAddress + offset;
}

void UI_MemoryScanner::DrawAddressCell(
    uintptr_t address, int rowIndex)
{
    char buf[32];
    snprintf(buf, sizeof(buf), "0x%llX", 
        (unsigned long long)address);

    float buttonWidth = ImGui::CalcTextSize("Copy").x + 
        ImGui::GetStyle().FramePadding.x * 2.0f;

    float columnRight = ImGui::GetCursorPosX() + 
...
void UI_MemoryScanner::DrawDiffResults()
{
    ...
    for (int d = 0; d < (int)diffs.size(); ++d)
    {
        ...
        bool clicked = ImGui::Selectable("##row", 
            d == m_SelectedRow, flags, { 0.0f, 0.0f });
...
std::string UI_MemoryScanner::FormatTypedValue(uint64_t raw, ScanDataType dt)
{
    ...
    case ScanDataType::Int8:
    {
        snprintf(buf, sizeof(buf), "%d (0x%02X)", 
            (int8_t)raw, (uint8_t)raw);   
        break;
    }

    case ScanDataType::UInt8:
    {
        snprintf(buf, sizeof(buf), "%u (0x%02X)",
    ...
    {
        snprintf(buf, sizeof(buf), "%d (0x%04X)", 
            (int16_t)raw, (uint16_t)raw);  
        break;
    }

    case ScanDataType::UInt16:
    {
        snprintf(buf, sizeof(buf), "%u (0x%04X)",
    ...
    {
        snprintf(buf, sizeof(buf), "%d (0x%08X)", 
            (int32_t)raw, (uint32_t)raw);  
        break;
    }

    case ScanDataType::UInt32:
    {
        snprintf(buf, sizeof(buf), "%u (0x%08X)", 
    ...
    case ScanDataType::Float32: 
    {
        ...
        snprintf(buf, sizeof(buf), "%.6f (0x%08X)", f, (uint32_t)raw); 
        break;
    }

    default:
    {
        snprintf(buf, sizeof(buf), "0x%02X", (uint8_t)raw); 
        break;
    }
    }
...
void UI_MemoryScanner::DrawTypedResults()
{
    ...
    for (int d = 0; d < (int)matches.size(); ++d)
    {
        ...
        this->DrawAddressCell(this->ResolveAddress(
            session, match.Offset), d);
        ...
        ImGui::TextDisabled("%s", this->FormatTypedValue(
            match.ValueBefore, dt).c_str());
...
uint64_t UI_MemoryScanner::ParseValueBuf(
    const char* buf, ScanDataType type) const
{
...
const UI_MemoryScanner::ModeEntry& 
    UI_MemoryScanner::CurrentMode() const 
{ 
    return s_Modes[m_ModeIndex]; 
}

ScanDataType UI_MemoryScanner::CurrentDataType() const 
{ 
    return s_DataTypes[m_DataTypeIndex].Type; 
}

bool UI_MemoryScanner::IsTypedMode() const 
{ 
    return this->CurrentDataType() != ScanDataType::Bytes; 
}
