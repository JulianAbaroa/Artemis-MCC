...
void UI_Logs::Draw()
{
    ...
    this->FirstDraw();
    this->ApplyResetIfRequested("Logs");

    ImGuiWindowFlags flags = ImGuiWindowFlags_None;
    if (UI_Tab::s_Locked) flags |= ImGuiWindowFlags_NoMove;

    ImGui::Begin("Logs", &m_Visible, flags);
...
LogFilterState UI_Logs::DrawTopBar()
{
...
void UI_Logs::DrawClearButton(bool isFiltering, std::string& searchStr)
{
    ...
    if (ImGui::Button(clearBtnLabel))
    {
        if (isFiltering)
        {
            ...
            m_Deps.System_Logs.RemoveLogsIf([&](const LogEntry& entry) {
...
void UI_Logs::DrawCopyButton(bool isFiltering, std::string& searchStr)
{
    ...
    if (ImGui::Button(copyBtnLabel))
    {
        ...
        m_Deps.State_Logs.ForEachLog([&](const LogEntry& entry) {
            if (isFiltering)
            {
                std::string textLower = entry.FullText;
                std::transform(textLower.begin(), textLower.end(), textLower.begin(), ::tolower);
...
std::vector<int> UI_Logs::GetFilteredIndices(const LogFilterState& filter)
{
    ...
    int totalLogs = static_cast<int>(m_Deps.State_Logs.GetTotalLogs());
    filtered.reserve(totalLogs / 2);

    for (int i = 0; i < totalLogs; i++)
    {
        LogEntry entry = m_Deps.State_Logs.GetLogAt(i);
        std::string textLower = entry.FullText;
...
void UI_Logs::DrawScrollingRegion(const LogFilterState& filter)
{
    ...
    while (clipper.Step())
    {
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        {
            ...
            LogEntry entry = m_Deps.State_Logs.GetLogAt(realIndex);
...
void UI_Logs::DrawLogLine(int realIndex, const LogEntry& entry, bool& logClickedThisFrame)
{
...
void UI_Logs::DrawLogMessage(const std::string& message)
{
...
void UI_Logs::HandleLogInteraction(int realIndex, const LogEntry& entry, bool& logClickedThisFrame)
{
    ...
    if (ImGui::IsItemClicked(ImGuiMouseButton_Right))
    {
        if (this->IsIndexSelected(realIndex))
        {
            ...
            {
                selectedText += m_Deps.State_Logs.GetLogAt(j).FullText + "\n";
            }
...
void UI_Logs::DrawSearchBar(char* buffer, size_t bufferSize)
{
...
bool UI_Logs::IsIndexSelected(int index) const
{
