...
void UI_Settings::DrawUserPreferences()
{
	if (!m_IsInitialized)
	{
	...
	if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
	{
		if (usePreferences)
		{
			ImGui::SetTooltip("Defines if Artemis control panel is opened on MCC start.");
		}
		else
		{
...
void UI_Settings::DrawHotkeysTable()
{
	ImGui::BeginGroup();
	ImVec2 p_min = ImGui::GetCursorScreenPos();
	...
	if (ImGui::BeginTable("HotkeysTable", 2, m_TableFlags, ImVec2(tableWidth, 0)))
	{
		...
		this->DrawHotkeyRow("Toggle Menu", "CTRL + 1", "Show or hide the Artemis Control Panel.");
		this->DrawHotkeyRow("Emergency Reset", "CTRL + 2", "Centers the window if it gets lost off-screen.");
		this->DrawHotkeyRow("Lock Menu", "CTRL + 3", "Lock the menu position and disable mouse dragging.");
...
void UI_Settings::DrawDataPersistence()
{
...
void UI_Settings::DrawSystemDirectories()
{
	ImVec2 p_min = ImGui::GetCursorScreenPos();
...
void UI_Settings::DrawHotkeyRow(const char* label, const char* keys, const char* tooltip)
{
	ImGui::TableNextRow();
...
void UI_Settings::DrawPathField(const char* label, const std::string& path, float widthOffset)
{
...
void UI_Settings::DrawPersistencePopups()
{
...
void UI_Settings::DrawConfirmDisableAppData()
{
...
void UI_Settings::DrawDeleteAllAppData()
{
...
std::string UI_Settings::GetFriendlyGameName(const std::string& path)
{
