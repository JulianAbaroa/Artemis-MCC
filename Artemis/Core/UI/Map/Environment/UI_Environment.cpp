...
{
    ImGui::Text("Physics instances: %d", static_cast<int>(
        m_CachedInstances.size()));
...
void UI_Environment::DrawSelectionPanel(const MapSelection& selection) const
{
    ...
    if (i < 0 || i >= static_cast<int32_t>(m_CachedInstances.size()))
    {
        return;
    }
    ...
    const ActivePhysicsInstance& instance) const
{
    ...
    ImGui::Text("Min: %.2f, %.2f, %.2f", instance.CollBoundsMin[0],
        instance.CollBoundsMin[1], instance.CollBoundsMin[2]);

    ImGui::Text("  Max: %.2f, %.2f, %.2f", instance.CollBoundsMax[0],
        instance.CollBoundsMax[1], instance.CollBoundsMax[2]);
    ...
    if (instance.Phmo)
    {
        ...
        ImGui::Text("Rigid bodies: %d", static_cast<int>(
            instance.Phmo->RigidBodies.size()));

        for (int32_t i = 0; i < static_cast<int32_t>(
            instance.Phmo->RigidBodies.size()); ++i)
        {
            const auto& rb = instance.Phmo->RigidBodies[i];

            ImGui::Spacing();

            ImGui::Text("[%d] Node %d | Shapes: %d", i, rb.NodeIndex,
            ...
            for (const auto& shape : rb.Shapes)
            {
                ...
                {
                    ImGui::Text("Sphere r=%.2f", shape.Sphere.Radius);
                    break;
                }

                case PhmoShapeType::Pill:
                {
                    ImGui::Text("Pill r=%.2f", shape.Pill.Radius);
                    break;
                }
    ...
    for (int32_t j = 0; j < static_cast<int32_t>(
        instance.WorldRigidBodies.size()); ++j)
    {
        const auto& wrb = instance.WorldRigidBodies[j];

        ImGui::Text("[%d] Shapes: %d | BSphere r=%.2f", j,
            (int)wrb.Shapes.size(), wrb.BoundingSphereRadius);

        for (int32_t k = 0; k < static_cast<int32_t>(
            wrb.Shapes.size()); ++k)
        {
...
{
    for (int32_t i = 0; i < static_cast<int32_t>(
        m_CachedInstances.size()); ++i)
    {
        const auto& instance = m_CachedInstances[i];

        bool selected = (selection.Type == 
            MapSelectionType::PhysicsInstance && selection.Index == i);

        ImU32 color = selected ? 
            IM_COL32(100, 220, 255, 255) : IM_COL32(60, 180, 220, 180);

        ImU32 fillColor = selected ? 
            IM_COL32(100, 220, 255, 60) : IM_COL32(60, 180, 220, 25);

        if (!instance.WorldRigidBodies.empty())
        {
            for (const auto& wrb : instance.WorldRigidBodies)
            {
                this->DrawWorldRigidBody(draw, transform, 
                    wrb, color, fillColor);
            }
        }
        else
        {
            this->DrawCollFallback(draw, transform, instance, 
                color, fillColor);
        }

        ImVec2 center = UI_MapHelpers::ToCanvas(instance.Position[0], 
            instance.Position[1], transform);
    ...
    ImU32 color, ImU32 fillColor) const
{
    ...
    {
        this->DrawWorldShape(draw, transform, ws, color, fillColor);
    }
}
    ...
    ImU32 color, ImU32 fillColor) const
{
    ...
    case WorldShapeType::Sphere:
    {
        ImVec2 center = UI_MapHelpers::ToCanvas(ws.SphereCenter[0], 
            ws.SphereCenter[1], transform);
    ...
    case WorldShapeType::Pill:
    {
        ...
        ImVec2 b = UI_MapHelpers::ToCanvas(ws.PillBottom[0], 
            ws.PillBottom[1], transform);

        ImVec2 t = UI_MapHelpers::ToCanvas(ws.PillTop[0], 
            ws.PillTop[1], transform);
    ...
    case WorldShapeType::Box:
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            pts[i] = UI_MapHelpers::ToCanvas(ws.BoxCorners[i][0], 
                ws.BoxCorners[i][1], transform);
        }
        ...
        for (int i = 0; i < 8; ++i)
        {
            while (hullSize >= 2 && cross2d(hull[hullSize - 2], 
                hull[hullSize - 1], pts[idx[i]]) <= 0) 
            { 
                --hullSize;
            }
        ...
        for (int i = 6; i >= 0; --i)
        {
            while (hullSize >= lower && cross2d(hull[hullSize - 2],
                hull[hullSize - 1], pts[idx[i]]) <= 0)
            {
                --hullSize;
            }
        ...
        if (hullSize >= 3)
        {
            ...
            draw->AddPolyline(hull, hullSize, color, 
    ...
    case WorldShapeType::Polyhedron:
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            while (hull.size() >= 2 && cross2d(hull[hull.size() - 2],
                hull[hull.size() - 1], pts[idx[i]]) <= 0)
            {
                hull.pop_back();
            }
        ...
        if (static_cast<int>(hull.size()) >= 3)
        {
            ...
            draw->AddPolyline(hull.data(), static_cast<int>(hull.size()),
