...
System_EventRegistry::System_EventRegistry()
{
	this->InitializeDefaultRegistry();
}

void System_EventRegistry::SaveEventRegistry()
{
...
void System_EventRegistry::LoadEventRegistry()
{
	std::string path = g_pState->Infrastructure->Settings->GetAppDataDirectory() + "\\event_weights.cfg";
...
void System_EventRegistry::InitializeDefaultRegistry()
{
...
EventInfo System_EventRegistry::BuildEvent(EventType type)
{
	return { type, this->ResolveEventClass(type) };
}

EventClass System_EventRegistry::ResolveEventClass(EventType type)
{
	switch (type)
	{
	case EventType::Join:
