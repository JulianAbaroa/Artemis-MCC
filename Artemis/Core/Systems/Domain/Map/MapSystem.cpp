...
#include "Core/Common/MapOffsets.h"
...
TagInfo MapSystem::ResolveHandle(uint32_t handle) const
{
...
int64_t MapSystem::GetTagMetaOffset(const TagInfo& info) const
{
...
int64_t MapSystem::GetTagMetaOffsetByIndex(int32_t tagIndex) const
{
	if (tagIndex < 0 ||
...
FILE* MapSystem::OpenMapFile() const
{
...
Map_TagTableEntry MapSystem::ReadTagRef(FILE* file, int64_t tagRefOffset) const
{
...
int64_t MapSystem::ResolveTagOffset(const Map_TagTableEntry& tag) const
{
...
Map_TagBlock MapSystem::ReadTagBlock(FILE* file, int64_t blockHeaderOffset) const
{
	Map_TagBlock block{};
...
int64_t MapSystem::ResolveBlockOffset(const Map_TagBlock& block) const
{
...
int64_t MapSystem::Expand(uint32_t address) const
{
	if (address == 0 || address == 0xFFFFFFFF) return 0;

	// Equivalent to: address * 4 + ExpandMagic.
	return ((int64_t)address << 2) + MapMagics::k_ExpandMagic;
}
...
int64_t MapSystem::ToFileOffset(int64_t virtualAddress) const
{
...
int64_t MapSystem::ToDebugOffset(int64_t pointer) const
{
...
bool MapSystem::ReadHeader(FILE* file)
{
...
bool MapSystem::ReadIndexHeader(FILE* file)
{
...
bool MapSystem::ReadTagGroups(FILE* file, int64_t fileOffset, int32_t count)
{
...
bool MapSystem::ReadTags(FILE* file, int64_t fileOffset, int32_t count)
{
	if (count <= 0) return false;
...
bool MapSystem::ReadFileNames(FILE* f) const
{
...
std::string MapSystem::MagicToString(int32_t magic) const
{
	char str[5]{};
