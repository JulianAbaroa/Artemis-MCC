...
Proj System_ProjBuilder::Build(const ProjObject& proj)
{
    Proj out{};
...
ProjectileType System_ProjBuilder::DeriveType(
    const ProjObject& proj)
{
    if (!proj.BruteGrenade.empty())
    {
        return ProjectileType::BruteGrenade;
    }

    if (!proj.FireBombGrenade.empty())
    {
        return ProjectileType::FireBomb;
    }

    if (!proj.ConicalSpread.empty())
    {
        return ProjectileType::ConicalSpread;
    }
...
bool System_ProjBuilder::DeriveCanBounce(const ProjObject& proj)
{
