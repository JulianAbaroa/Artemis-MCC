#include "pch.h"
#include "Core/Systems/Core_System.h"
#include "Core/Systems/Interface/System_Debug.h"
#include "Core/Systems/Infrastructure/Engine/System_Scanner.h"
#include <psapi.h>
#include <vector>

uintptr_t System_Scanner::FindPattern(const Signature& sig, const wchar_t* moduleName)
{
    return FindPattern(sig.pattern, moduleName, sig.name);
}

uintptr_t System_Scanner::FindPattern(
    const char* pattern, const wchar_t* moduleName, const char* name)
{
    HMODULE hModule = GetModuleHandle(moduleName);

    if (!hModule) return 0;

    MODULEINFO modInfo;
    if (!GetModuleInformation(GetCurrentProcess(), hModule, &modInfo, sizeof(MODULEINFO))) 
    {
        return 0;
    }

    return Scan((uintptr_t)modInfo.lpBaseOfDll, modInfo.SizeOfImage, pattern, name);
}


uintptr_t System_Scanner::Scan(uintptr_t base, size_t size, 
    const char* pattern, const char* name)
{
    auto patternToByte = [](const char* pattern)
        {
            auto bytes = std::vector<int>{};
            char* start = const_cast<char*>(pattern);
            char* end = const_cast<char*>(pattern) + strlen(pattern);

            for (char* current = start; current < end; ++current)
            {
                if (*current == ' ') continue;

                if (*current == '?')
                {
                    current++;
                    if (*current == '?') current++;
                    bytes.push_back(-1);
                }
                else
                {
                    bytes.push_back(strtoul(current, &current, 16));
                }
            }
            return bytes;
        };

    auto patternBytes = patternToByte(pattern);
    if (patternBytes.empty()) return 0;

    uint8_t* scanStart = reinterpret_cast<uint8_t*>(base);
    size_t patternSize = patternBytes.size();
    int* patternData = patternBytes.data();

    for (size_t i = 0; i < size - patternSize; ++i)
    {
        bool found = true;
        for (size_t j = 0; j < patternSize; ++j)
        {
            if (scanStart[i + j] != patternData[j] && patternData[j] != -1)
            {
                found = false;
                break;
            }
        }

        if (found)
        {
			uintptr_t foundAddress = reinterpret_cast<uintptr_t>(&scanStart[i]);

			// g_pSystem->Debug->Log("[Scanner] INFO: Pattern '%s' found at address 0x%p.", 
            //     name, (void*)foundAddress);

            return foundAddress;
        }
    }

    return 0;
}