...
uintptr_t System_AOBScanner::FindPattern(const Signature& sig, const wchar_t* moduleName)
{
    return FindPattern(sig.pattern, moduleName, sig.name);
}

uintptr_t System_AOBScanner::FindPattern(
    const char* pattern, const wchar_t* moduleName, const char* name)
{
...
uintptr_t System_AOBScanner::Scan(uintptr_t base, size_t size,
    const char* pattern, const char* name)
{
    ...
    for (size_t i = 0; i < size - patternSize; ++i)
    {
        ...
        if (found)
        {
			uintptr_t foundAddress = reinterpret_cast<uintptr_t>(&scanStart[i]);

			// g_pSystem->Debug->Log("[Scanner] INFO: Pattern '%s' found at address 0x%p.", 
            //     name, (void*)foundAddress);
