...
uintptr_t System_AOBScanner::FindPattern(
    const Signature& sig, const wchar_t* moduleName)
{
    return this->FindPattern(sig.pattern, moduleName, sig.name);
}

uintptr_t System_AOBScanner::FindPattern(const char* pattern, 
    const wchar_t* moduleName, const char* name)
{
    ...
    return this->Scan((uintptr_t)modInfo.lpBaseOfDll, 
        modInfo.SizeOfImage, pattern, name);
}
    ...
    const char* pattern, const char* name)
{
    ...
    for (size_t i = 0; i < size - patternSize; ++i)
    {
        ...
        {
            if (scanStart[i + j] != patternData[j] && 
                patternData[j] != -1)
            {
                found = false;
                break;
