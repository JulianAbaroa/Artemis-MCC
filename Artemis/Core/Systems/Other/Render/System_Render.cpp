...
DX11Addresses System_Render::GetVtableAddresses() const
{
    ...
    {
        m_Deps.System_Logs.Log("[DXUtil] ERROR: RegisterClassEx failed");
        return addr;
    }
    ...
    if (!hWnd)
    {
        m_Deps.System_Logs.Log("[DXUtil] ERROR: CreateWindowEx failed");
    ...
    {
        m_Deps.System_Logs.Log("[DXUtil] ERROR: D3D11CreateDeviceAndSwapChain failed");
    }
...
void System_Render::Initialize(IDXGISwapChain* pSwapChain)
{
    ...
    if (!m_Deps.State_Render.IsImGuiInitialized())
    {
        ...
        m_Deps.System_Logs.Log("[RenderSystem] INFO: ImGui Initialized for the first time.");
    }
    else
    {
        ImGui_ImplDX11_InvalidateDeviceObjects();
        ImGui_ImplDX11_CreateDeviceObjects();
        m_Deps.System_Logs.Log("[RenderSystem] INFO: Device Objects Refreshed (Resize).");
    }
    ...
    if (SUCCEEDED(pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&pBackBuffer)))
    {
...

        {
            ...
            depthDesc.Width = static_cast<UINT>(m_Deps.State_Render.GetWidth());
            depthDesc.Height = static_cast<UINT>(m_Deps.State_Render.GetHeight());
            ...
            if (SUCCEEDED(device->CreateTexture2D(&depthDesc, nullptr, &pDepthTex)))
            {
                ...
                if (SUCCEEDED(device->CreateDepthStencilView(pDepthTex, &dsvDesc, &dsv)))
                {
                    m_Deps.State_Render.SetDSV(dsv);
                }
                else
                {
                    m_Deps.System_Logs.Log("[RenderSystem] ERROR: CreateDepthStencilView failed.");
                }
                pDepthTex->Release();
            }
            else
            {
                m_Deps.System_Logs.Log("[RenderSystem] ERROR: depth CreateTexture2D failed.");
            }
        }
    }
}
...
void System_Render::Shutdown()
{
    ...
    m_Deps.System_Logs.Log("[RenderSystem] INFO: Shutdown complete.");
}
...
void System_Render::BeginFrame(IDXGISwapChain* pSwapChain)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    bool anyVisible = UI_Tab::s_VisibleCount > 0 || UI_Launcher::s_IsVisible;
