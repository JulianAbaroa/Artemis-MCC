...
namespace TagName
{
	...
	namespace Vehicle
	{
		...
		// TODO: Get the driver turret tag name.
		...
		constexpr const char* ShadeTurret = "objects\\vehicles\\covenant\\shade\\shade";

		constexpr const char* Warthog = "objects\\vehicles\\human\\warthog\\warthog";
		constexpr const char* WarthogTurret = "objects\\vehicles\\human\\warthog\\weapons\\warthog_chaingun\\warthog_chaingun";
		...
		inline bool IsVehicle(const std::string& tag)
		{
			return tag == Banshee || tag == Falcon ||
				tag == FalconTurretRight || tag == FalconTurretLeft ||
				tag == FalconGrenadeRight || tag == FalconGrenadeLeft ||
				tag == Ghost || tag == Mongoose ||
				tag == Revenant || tag == Scorpion ||
				tag == ScorpionTurret || tag == ShadeTurret ||
				tag == Warthog || tag == WarthogTurret ||
