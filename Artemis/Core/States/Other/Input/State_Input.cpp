...
InputRequest State_Input::GetNextRequest() const 
{ 
	return m_NextRequest; 
}

bool State_Input::IsProcessing() const 
{ 
...
void State_Input::SetProcessing(bool processing) 
{ 
	m_IsProcessing.store(processing); 
}

void State_Input::EnqueueRequest(const InputRequest& request, 
	bool uniqueRequest)
{
	std::lock_guard<std::mutex> lock(m_Mutex);
