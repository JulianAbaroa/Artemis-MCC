...
bool MapScenState::HasScen(const std::string& tagName) const
{
    std::lock_guard<std::mutex> lock(m_Mutex);
    return m_Scens.count(tagName) > 0;
}

const ScenObject* MapScenState::GetScen(const std::string& tagName) const
{
    std::lock_guard<std::mutex> lock(m_Mutex);
...
void MapScenState::AddScen(const std::string& tagName, ScenObject data)
{
