...
void State_MapBloc::LogRawData(const std::string& tagName, const BlocObject& bloc)
{
    const auto& d = bloc.Data;

    g_pSystem->Debug->Log("=== BLOC RAW: %s ===", tagName.c_str());
    ...
    g_pSystem->Debug->Log("    Flags_2:                       0x%04X", d.Flags_2);
    g_pSystem->Debug->Log("      DoesNotBlockAreaOfEffect:    %d", (d.Flags_2 >> 0) & 1);

    // --- AI Properties ---
    g_pSystem->Debug->Log("  [AiProperties: %zu]", bloc.AiProperties.size());
    for (size_t i = 0; i < bloc.AiProperties.size(); ++i)
    {
        const auto& ai = bloc.AiProperties[i];
    ...
    // --- MultiplayerObject ---
    g_pSystem->Debug->Log("  [MultiplayerObject: %zu]", bloc.MultiplayerObject.size());
    for (size_t i = 0; i < bloc.MultiplayerObject.size(); ++i)
    {
        const auto& mp = bloc.MultiplayerObject[i];
    ...
    // --- MetagameProperties ---
    g_pSystem->Debug->Log("  [MetagameProperties: %zu]", bloc.MetagameProperties.size());
    for (size_t i = 0; i < bloc.MetagameProperties.size(); ++i)
    {
        const auto& mg = bloc.MetagameProperties[i];
        g_pSystem->Debug->Log("    [%zu] Flags:0x%02X Unit:%u Classification:%u Points:%d",
            i,
            (uint32_t)mg.Flags,
            (uint32_t)mg.Unit,
            (uint32_t)mg.Classification,
            (int32_t)mg.Points);
    }

    g_pSystem->Debug->Log("=== END BLOC: %s ===", tagName.c_str());
}