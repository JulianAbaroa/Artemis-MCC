...
bool MapBlocState::HasBloc(const std::string& tagName) const
{
    std::lock_guard<std::mutex> lock(m_Mutex);
    return m_Blocs.count(tagName) > 0;
}

const BlocObject* MapBlocState::GetBloc(const std::string& tagName) const
{
    std::lock_guard<std::mutex> lock(m_Mutex);
...
void MapBlocState::AddBloc(const std::string& tagName, BlocObject data)
{
