	...
	LPVOID pData, PUINT pcbSize, UINT cbSizeHeader)
{
	...
	if (isValid && raw->header.dwType == RIM_TYPEMOUSE)
	{
		...
		else if (s_Instance->m_Deps.State_FlyCamera.IsActive())
		{
			s_Instance->m_Deps.State_FlyCamera.AccumulateMouseDelta(
				(float)raw->data.mouse.lLastX, (float)raw->data.mouse.lLastY);
