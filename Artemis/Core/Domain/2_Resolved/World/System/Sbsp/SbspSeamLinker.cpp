...
import :SbspSeamLinker;

namespace
{
    using Cluster = Resolved::World::Type::Sbsp::Cluster;
    using CrossLink = Resolved::World::Type::Sbsp::CrossLink;
}
...
namespace Resolved::World::System
{
    ...
    auto SbspSeamLinker::LinkSeams(std::vector<ResolvedSbsp>& geometries,
        const std::vector<const SbspObject*>& sbspObjects,
        float threshold) -> void
    {
        const std::int32_t sbspCount = static_cast<std::int32_t>(geometries.size());

        if (sbspCount < 2) return; // Nothing to link.

        const float thresholdSq = threshold * threshold;

        for (std::int32_t iA = 0; iA < sbspCount; ++iA)
        {
            const SbspObject* objA = sbspObjects[iA];
            if (!objA) continue;

            ResolvedSbsp& geoA = geometries[iA];
            const std::int32_t clusterCountA = static_cast<std::int32_t>(geoA.Clusters.size());

            for (std::int32_t iB = 0; iB < sbspCount; ++iB)
            {
                if (iB == iA) continue;
                ...
                const ResolvedSbsp& geoB = geometries[iB];
                const std::int32_t clusterCountB = static_cast<std::int32_t>(geoB.Clusters.size());
                ...
                for (const auto& seamA : objA->StructureSeams)
                {
                    ...
                    {
                        const std::int32_t clusterIdxA = scA.ClusterIndex;
                        ...
                        for (const auto& seamB : objB->StructureSeams)
                        {
                            ...
                            {
                                const std::int32_t clusterIdxB = scB.ClusterIndex;

                                if (clusterIdxB < 0 || clusterIdxB >= clusterCountB)
                                    continue;

                                const Vec3 centroidB = {
                                    scB.Centroid.X,
                                    scB.Centroid.Y,
                                    scB.Centroid.Z
                                };
                                ...
                                Cluster& clusterA = geoA.Clusters[clusterIdxA];
    ...
    auto SbspSeamLinker::DistanceSquared(const Vec3& a, const Vec3& b) -> float
    {
