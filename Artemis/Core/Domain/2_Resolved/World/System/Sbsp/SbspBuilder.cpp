...
namespace
{
    ...
    using Sbsp_MarkersEntry = Map::Tag::Type::Sbsp::Structure::Sbsp_MarkersEntry;
    using Sbsp_CollisionMaterialsEntry = Map::Tag::Type::Sbsp::Structure::Sbsp_CollisionMaterialsEntry;
    using Sbsp_InstancedGeometrySubGroupsObject = Map::Tag::Type::Sbsp::Object::Sbsp_InstancedGeometrySubGroupsObject;
    using Sbsp_ClusterPortalsObject = Map::Tag::Type::Sbsp::Object::Sbsp_ClusterPortalsObject;
    using Sbsp_ClustersObject = Map::Tag::Type::Sbsp::Object::Sbsp_ClustersObject;
    using InstancedGeometryEntry = Resolved::World::Type::Sbsp::InstancedGeometryEntry;
}

namespace Resolved::World::System
{
    auto SbspBuilder::Build(const SbspObject& sbsp, std::int32_t sbspIndex) -> ResolvedSbsp
    {
        ResolvedSbsp out;
        out.TagName = sbsp.TagName;
        out.SbspIndex = sbspIndex;

        this->BuildWorldBounds(sbsp, out);
        this->BuildPortals(sbsp, out);
        this->BuildClusters(sbsp, out);
        this->BuildClusterAdjacency(sbsp, out);
    ...
    auto SbspBuilder::BuildWorldBounds(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
    ...
    auto SbspBuilder::BuildPortals(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
        const std::int32_t count = static_cast<std::int32_t>(sbsp.ClusterPortals.size());
        ...
        for (std::int32_t i = 0; i < count; ++i)
        {
            ...
            Portal p;
            p.PortalIndex = i;
            p.BackCluster = static_cast<std::int32_t>(src.BackCluster);
            p.FrontCluster = static_cast<std::int32_t>(src.FrontCluster);
            p.Centroid = std::bit_cast<Vec3>(src.Centroid);
            p.BoundingRadius = src.BoundingRadius;

            p.Vertices.reserve(src.Vertices.size());
            for (const auto& v : src.Vertices)
            {
                p.Vertices.push_back(std::bit_cast<Vec3>(v.Position));
            }
    ...
    //
    // We build the full Cluster here; adjacency edges are added in the next
    // pass so that all Cluster objects already exist when we link them.
    auto SbspBuilder::BuildClusters(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
        const std::int32_t count = static_cast<std::int32_t>(sbsp.Clusters.size());
        ...
        for (std::int32_t i = 0; i < count; ++i)
        {
            ...
            {
                c.PortalIndices.push_back(static_cast<std::int32_t>(pe.PortalIndex));
            }
    ...
    auto SbspBuilder::BuildClusterAdjacency(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
        const std::int32_t clusterCount = static_cast<std::int32_t>(out.Clusters.size());
        const std::int32_t portalCount = static_cast<std::int32_t>(out.Portals.size());

        for (std::int32_t ci = 0; ci < clusterCount; ++ci)
        {
            Cluster& cluster = out.Clusters[ci];

            for (std::int32_t portalIdx : cluster.PortalIndices)
            {
                if (portalIdx < 0 || portalIdx >= portalCount)
                    continue;

                const Portal& portal = out.Portals[portalIdx];

                // Check both sides of the portal; take whichever is not us
                for (std::int32_t neighbor : { portal.BackCluster, portal.FrontCluster })
                {
                    if (neighbor < 0 || neighbor >= clusterCount || neighbor == ci)
                        continue;

                    bool alreadyAdded = false;
                    for (std::int32_t existing : cluster.ConnectedClusters)
                    {
    ...
    auto SbspBuilder::BuildInstancedGeometry(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
        const std::int32_t instanceCount = static_cast<std::int32_t>(sbsp.InstancedGeometryInstances.size());
        const std::int32_t subGroupCount = static_cast<std::int32_t>(sbsp.InstancedGeometrySubGroups.size());

        out.InstancedGeometry.reserve(subGroupCount);

        for (std::int32_t si = 0; si < subGroupCount; ++si)
        {
            const Sbsp_InstancedGeometrySubGroupsObject& sg = sbsp.InstancedGeometrySubGroups[si];

            InstancedGeometryEntry entry{};
            entry.SubGroupIndex = si;
            entry.Center = std::bit_cast<Vec3>(sg.Center);
            ...
            if (!sg.Members.empty())
            {
                const std::int32_t nameIdx = static_cast<std::int32_t>(sg.Members[0].NameIndex);
    ...
    auto SbspBuilder::BuildCollisionMaterials(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
        const std::int32_t count = static_cast<std::int32_t>(sbsp.CollisionMaterials.size());
    ...
    auto SbspBuilder::BuildMarkers(const SbspObject& sbsp, ResolvedSbsp& out) -> void
    {
        const std::int32_t count = static_cast<std::int32_t>(sbsp.Markers.size());
        ...
        for (std::int32_t i = 0; i < count; ++i)
        {
            ...
            marker.Position = std::bit_cast<Vec3>(src.Position);
            marker.Rotation = std::bit_cast<Vec4>(src.Rotation);

            out.Markers.push_back(marker);
        }
    }

    auto SbspBuilder::RangeFCenter(const RangeF& x, const RangeF& y, const RangeF& z) -> Vec3
    {
        return
        {
