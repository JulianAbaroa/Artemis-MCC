...
Usage:
    python GenerateStructure.py input.xml output.h
"""
...
TYPE_INFO: dict[str, tuple[str, int]] = {
    ...
    "float64":  ("double",              8),
    "degree":   ("float",               4),
    "flags8":   ("std::uint8_t",             1),
    "flags16":  ("std::uint16_t",            2),
    "flags32":  ("std::uint32_t",            4),
    "flags64":  ("std::uint64_t",            8),
    "enum8":    ("std::uint8_t",             1),
    "enum16":   ("std::uint16_t",            2),
    "enum32":   ("std::uint32_t",            4),
    "stringid": ("std::uint32_t",            4),
    "vector3":  ("MapStructure::Primitive::Vec3",      12),
    "point3":   ("MapStructure::Primitive::Vec3",      12),
    ...
    "tagref":   ("MapStructure::Tag::Reference", 16),
    "dataref":  ("MapStructure::Data::Reference", 20),
}
        ...
        for i, (tag, child, offset) in enumerate(childrenInfo):
            ...
            if tag == "tagblock":
                ...
                if available >= 16 or available == 0:
                    cType, size = "MapStructure::Primitive::RangeD", 16
                else:
                    cType, size = "MapStructure::Primitive::RangeF", 8
            elif tag == "colorf":
                hasAlpha = child.get("alpha", "false").lower() == "true"
                cType, size = ("MapStructure::Primitive::ColorRGBA", 16) if hasAlpha else ("MapStructure::Primitive::ColorRGB", 12)
            ...
            if available > 0 and size > available:
                ...
                if available >= 8:   cType, size = "std::uint64_t", 8
                elif available >= 4: cType, size = "std::uint32_t", 4
                elif available >= 2: cType, size = "std::uint16_t", 2
                elif available >= 1: cType, size = "std::uint8_t", 1
...
def EmitField(f: Field, cursor: int, lines: list[str]) -> int:
    if f.CType == "_overlay_comment_":
        ...
        return cursor
    if f.Offset > cursor:
        lines.append(f"    std::uint8_t  _gap_0x{cursor:X}[0x{f.Offset - cursor:X}];")
        cursor = f.Offset
    if f.CType == "_pad_":
        lines.append(f"    std::uint8_t  _pad_0x{f.Offset:X}[0x4];  // {f.Comment}")
        return cursor + 4
    if f.CType == "_TagBlock_":
        lines.append(f"    // -> {f.EntryStruct}[]")
        lines.append(f"    MapStructure::Tag::Block   {f.Name};  // 0x{f.Offset:X}  {f.Comment}")
        return cursor + 12
...
def EmitStruct(s: Struct, lines: list[str]) -> None:
    ...
    if s.TotalSize > 0 and cursor < s.TotalSize:
        lines.append(f"    std::uint8_t  _tail_0x{cursor:X}[0x{s.TotalSize - cursor:X}];  // trailing pad")
    lines.append("};")
    if s.TotalSize > 0:
...
def Generate(xmlPath: str, outPath: str) -> None:
    ...
    lines = []
    lines.append(f"// Auto-generated by TagTranspiler (GenerateStructure.py)")
    lines.append(f"// Source XML: {os.path.basename(xmlPath)}")
    lines.append(f"// Root struct size: {total:#x}")
    lines.append(f"")

    lines.append(f"export module Map.Tag.Type:{prefix}.Structure;")
    lines.append(f"")
    ...
    lines.append("{")
    lines.append(f"    namespace MapStructure = Map::Reader::Type::Structure;")
    lines.append("}")
    lines.append(f"")
    ...
    lines.append(f"export namespace Map::Tag::Type::{prefix}::Structure")
    lines.append(f"{{")

    for s in parser.Structs:
        struct_lines = []
        EmitStruct(s, struct_lines)
    ...
    if outPath.endswith(".h"):
        outPath = outPath[:-2] + ".ixx"

    with open(outPath, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Generated Partition: {outPath}")

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python GenerateStructure.py input.xml output.h")
        sys.exit(1)
