...
def parse_log(filepath):
    ...
    re_tag_start  = re.compile(r'=== BLOC RAW: .+ ===')
    re_tag_end    = re.compile(r'=== END BLOC:')
    ...
    with open(filepath, encoding='utf-8', errors='replace') as f:
        for raw_line in f:
            ...
            if m_item and current_array:
                ...
                for kv in re.finditer(r'([\w.]+):(-?[\w.x]+)', pairs):
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python filter_bloc_fields.py <logfile.txt>")
        sys.exit(1)
