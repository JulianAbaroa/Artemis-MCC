...
def parse_log(filepath):
    ...
    re_tag_start  = re.compile(r'=== CTRL RAW: .+ ===')
    re_tag_end    = re.compile(r'=== END CTRL:')
...
def report(scalar_values, array_values):
    ...
    for field in sorted(scalar_values):
        ...
        if not is_always_zero(vals):
            non_zero = sorted(str(v) for v in vals if v != 0.0 and v != '0')
            print(f"  {field}")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python filter_ctrl_fields.py <logfile.txt>")
        sys.exit(1)
